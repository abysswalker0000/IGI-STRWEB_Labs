...
from main.models import Review, News, FAQ, Cart, CartItem, Order, OrderItem, Partner, Company
from django.shortcuts import render
...
def view_cart(request):
    cart = Cart.objects.get(user=request.user)
    cartproduct_set = CartItem.objects.filter(cart=cart)
    total_price = sum(cart_product.product.price * cart_product.quantity for cart_product in cartproduct_set)
...
def contacts_view(request):
    if request.method == 'POST':
        ...
        # Валидация URL по заданным критериям
        url_pattern = r'^(https?://).+\.(php|html)$'
        ...
        try:
            ...
            return render(request, 'settings.html', {
                'contacts': contacts,
                'url_error': 'Не удалось загрузить изображение по указанному URL.'
            })
        ...
        # Сохранение контакта без изображения, чтобы получить ID
        contact.save()

        # Сохранение изображения
        contact.image.save(image_name, image_content)
        contact.save()

        return redirect('contacts') 
    else:
        contacts = Contacts.objects.all()
        return render(request, 'settings.html', {'contacts': contacts})
